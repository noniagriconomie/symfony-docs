...
these statuses are called **places**. You can define the workflow like this::
    ...
    $singleState = true; // true if the subject can be in only one state for a given time
    $property = 'currentState' // subject property name where the stage is stored
...
you can retrieve a workflow from it and use it as follows::
    ...
    $workflow->apply($post, 'to_review'); // $post is now in state "reviewed"
